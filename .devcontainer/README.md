...
Reproducible environment for the polyglot monorepo: Bun, Rust, C (clang), Python, Lua, ShellCheck, shfmt, Ruff, stylua, luacheck.
...
After the container starts, `postCreateCommand` runs `bun install` and `bun run prepare` (git hooks). Then use `bun run dev`, `bun run lint`, `bun run build`, etc. as on the host.
